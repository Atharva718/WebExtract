...
def scrap(url, idx):
    ...
    # Filter out "Remove Ad" from the date list
    date = [i.get_text() for i in span if i.get_text() != "Remove Ad"]
...
#save data as json named by current date with formatted output
def json_dump(data):
    date = datetime.date.today().strftime("%B %d, %Y")
    with open('moneycontrol_'+str(date)+'.json', 'w') as outfile:
        json.dump(submission, outfile, indent=4, separators=(",", ": "))
